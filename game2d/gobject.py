...
from colors_geom.geom import Point2, Matrix
...
def is_color(c):
    ...
    As with Turtles, colors may be colors_geom objects or strings.  They may also be
    ...
    """
    import colors_geom
    if type(c) in [colors_geom.RGB, colors_geom.HSV]:
        return True
    ...
    return type(c) == str and (colors_geom.is_tkcolor(c) or colors_geom.is_webcolor(c))
...
class GObject(object):
    ...
    def linecolor(self):
        ...
        assign color an `RGB` or `HSV` object from `colors_geom`, or a string with a valid
        color name. If you chose either of these alternate representations (a string or
        an object from `colors_geom`), Python will automatically convert the result into
        a 4-element list.
    ...
    def linecolor(self,value):
        ...
        elif type(value) in [colors_geom.RGB, colors_geom.HSV]:
            value = value.glColor()
        elif type(value) == str:
            if value[0] == '#':
                value = colors_geom.RGB.CreateWebColor(value).glColor()
            else:
                value = colors_geom.RGB.CreateName(value).glColor()
